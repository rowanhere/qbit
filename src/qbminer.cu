...
__device__ static bool hash_words_meet_target(const uint32_t h[8], const uint32_t target[8]) {
  ...
  if (target[0] == 0 && h[7] != 0) return false;
  #pragma unroll
  for (int i = 0; i < 32; i++) {
...
static std::vector<uint8_t> flip_word_bytes(const std::vector<uint8_t> &in) {
  std::vector<uint8_t> out;
  if (in.size() % 4 != 0) return out;
  out.reserve(in.size());
...
static std::vector<uint8_t> make_header76(const Job &job, const std::string &ex1, const std::string &ex2) {
  ...
  auto merkle_words = flip_word_bytes(merkle);

  std::vector<uint8_t> preflip;
  preflip.reserve(76);
  preflip.insert(preflip.end(), version.begin(), version.end());
  preflip.insert(preflip.end(), prev.begin(), prev.end());
  preflip.insert(preflip.end(), merkle_words.begin(), merkle_words.end());
  preflip.insert(preflip.end(), ntime.begin(), ntime.end());
  preflip.insert(preflip.end(), nbits.begin(), nbits.end());

  return flip_word_bytes(preflip);
}
    ...
    for (uint64_t start64 = 0; start64 < 0x100000000ULL; start64 += batch) {
      ...
      if (h_res.found) {
        ...
        if (opt.debug_shares && event_log.is_open()) {
          ...
          event_log << "nonce_submit_mode=qbit_reference_u32_hex\n";
          event_log << "nonce_u32=" << h_res.nonce << "\n";
          event_log << "header_mode=qbit_reference_wordflip\n";
